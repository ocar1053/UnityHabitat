...
float4 Output(float depth01, float3 normal)
{
    ...
    else if (_OutputMode == 5) // PhysicalDepth
    {
        float near = _ProjectionParams.y;
        float far = _ProjectionParams.z;
        float depthClip = (1.0 - depth01) * 2.0 - 1.0;
        float physicalDistance = (2.0 * near * far) / (far + near - depthClip * (far - near));
        return float4(physicalDistance, physicalDistance, physicalDistance, 1);
    }
    else if (_OutputMode == 6) // PhysicalDepth
    {
        float depth16 = saturate(depth01) * 65535.0; // 得到 0~65535 的值
        float highByte = floor(depth16 / 256.0) / 255.0;  // 高位歸一化到 0~1
        float lowByte  = (depth16 - floor(depth16 / 256.0) * 256.0) / 255.0; // 低位歸一化到 0~1
        return float4(highByte, lowByte, depth01, 1);
    }
